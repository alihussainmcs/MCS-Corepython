...
An object which will return data, one element at a time. => Iterators are everywhere in Python. 
...
collectively called the iterator protocol. => An object is called iterable if we can get an iterator from it. Most of 
built-in containers in Python like: list, tuple, string etc. are iterables. => iter() function (which in turn calls 
