...
Data structure is a storage that is used to store and organize data. It is a way of arranging data on a computer so that
...
Note: Data structure and data types are slightly different. Data structure is the collection of data types arranged in a
specific order.
...
It works just like a pile of plates where the last plate kept on the pile will be removed first. To learn more, visit
Stack Data Structure.


3. Queue Data Structure
Unlike stack, the queue data structure works in the FIFO principle where first element stored in the queue will be
removed first.
...
Similar to a graph, a tree is also a collection of vertices and edges. However, in tree data structure, there can only
be one edge between two vertices.
...
It can be traversed on a single run. That is, if we       It requires multiple runs. That is, if we start from the first
start from the first element, we can traverse all         element it might not be possible to traverse all the
the elements sequentially in a single pass.	              elements in a single pass.
...
Why Data Structure?
Knowledge about data structures help you understand the working of each data structure. And, based on that you can
