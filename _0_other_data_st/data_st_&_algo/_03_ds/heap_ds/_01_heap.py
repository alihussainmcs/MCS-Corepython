...
always greater than its child node/s and the key of the root node is the largest among all other nodes. This property is
also called max heap property.
always smaller than the child node/s and the key of the root node is the smallest among all other nodes. This property
is also called min heap property.

This type of data structure is also called a binary heap.


Heapify
Heapify is the process of creating a heap data structure from a binary tree. It is used to create a Min-Heap or a
Max-Heap.
...
heapify
Swap if necessary
Repeat steps 3-7 until the subtrees are also heaped.
Algorithm
...
def heapify(arr1, n, i):
    largest = i
    l1 = 2 * i + 1
    r = 2 * i + 2

    if l1 < n and arr1[i] < arr1[l1]:
        largest = l1
