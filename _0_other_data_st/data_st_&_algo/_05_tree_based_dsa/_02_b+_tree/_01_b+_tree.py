...
The leaf nodes of a B+ tree are linked together in the form of a singly linked lists to make the search queries more
    efficient.

B+ Tree are used to store the large amount of data which can not be stored in the main memory. Due to the fact that,
size of main memory is always limited, the internal nodes (keys to access records) of the B+ tree are stored in the main
...
3	Searching for some data is a slower process since data can      Searching is comparatively faster as data can only
        be found on internal nodes as well as on the leaf nodes.	 be found on the leaf nodes.
