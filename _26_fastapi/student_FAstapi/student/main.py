...
from service import service_insert, service_retrive_one, service_update, service_delete, service_subject
import models
...
def insert_data(std: Student, db: Session = Depends(get_db)):
    ...
    new_user = models.Student_model(name=std.name, Kannada=std.Kannada, English=std.English, Hindhi=std.Hindhi,
                                    Maths=std.Maths, Science=std.Science, Social=std.Social, status=status,
                                    chance=chance)
