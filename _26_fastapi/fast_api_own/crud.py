...
from _26_fastapi.fast_api_own.exeptions import CarInfoInfoAlreadyExistError, CarInfoNotFoundError
...
def create_car(session: Session, car_info: CreateAndUpdateCar) -> CarInfo:
    car_details = session.query(CarInfo).filter(CarInfo.manufacturer == car_info.manufacturer,
                                                CarInfo.modelName == car_info.modelName).first()
